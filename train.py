...
# ])
# train_transform = transforms.Compose([
#     transforms.RandomResizedCrop(128, scale=(0.08, 1.0)), # Much better than standard Resize
#     transforms.RandomHorizontalFlip(),
#     transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4),
#     transforms.RandomGrayscale(p=0.2),
#     transforms.ToTensor(),
#     transforms.RandomErasing(p=0.2), # Forces model to find multiple features
#     transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
# ])
train_transform = transforms.Compose([
    # 1. Force the model to recognize parts (eyes, nose, crumb)
    transforms.RandomResizedCrop(128, scale=(0.4, 1.0)), 
    
    # 2. Prevent background-based memorization
    transforms.RandomHorizontalFlip(),
    transforms.RandomVerticalFlip(p=0.1), # Muffins look similar upside down!
    
    # 3. Stop color-based shortcuts
    transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4),
    ...
    # 5. The "Secret Weapon": Random Erasing
    # This forces the model to find a SECONDARY feature if the primary one is blocked.
    transforms.RandomErasing(p=0.3, scale=(0.02, 0.2), ratio=(0.3, 3.3)),
    
    # 6. Keep standard normalization
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
])
...
def train():
    ...
    val_dataloader = DataLoader(val_table, batch_size=BATCH_SIZE, shuffle=True, num_workers=0)

    model = ResNet18Classifier(num_classes=NUM_CLASSES).to(device)
    criterion = nn.CrossEntropyLoss(label_smoothing=0.15)
    # optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
    optimizer = optim.SGD(model.parameters(),lr=LEARNING_RATE,momentum = 0.9,nesterov=True,weight_decay=1e-3)
